...
if(isset($_GET['recuperar'])){
	...
	if(isset($_POST['email'])){
		 	...
		 	$correu = $_POST['email'];
            $sql ="SELECT ID,Correu, Nom from professor where Correu ='$correu'";
       ...
       $sql1 = "INSERT INTO recuperacio_professor (IDprofessor, Data, Codi, Utilitzat) values ($idalumne,'$date',$randomNumber,0)";
       $conn->query($sql1);
...
try {
    ...
    $mail->Body    = 'Has fet una recuperació de contrasenya.</br></br> El número de recuperació: '.$randomNumber.'</br></br> Vés a aquest enllaç <a href="http://localhost/WEB/recuperar_password.php?recuperar='.$idalumne.'">recuperar contrasenya</a>';
...
if(isset($_POST['codi'])){
	    ...
	    $conn = mysqli_connect('localhost:3308','root','','racoestudi');
            $sql ="SELECT count(*) as si from recuperacio_professor where Utilitzat = 0 and Codi = $codi and IDprofessor =$idalumne";
            $result = $conn->query($sql);
            $count =0;
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
        ...
        if($count == 0){
        	...
        	$i = -4;
        	$conn = mysqli_connect('localhost:3308','root','','racoestudi');
		       $sql1 = "UPDATE recuperacio_professor SET utilitzat=1 WHERE Codi=$codi and IDprofessor =$idalumne";
		       $conn->query($sql1);
		     ...
		     $conn = mysqli_connect('localhost:3308','root','','racoestudi');
		       $sql1 = "UPDATE professor SET Contrasenya='$contrasenya' WHERE ID =$idalumne";
		       $conn->query($sql1);
...
<body>
    ...
    <div class="login-clean">
    	<?php
    	if($i==0){
    		echo"<form method='post' id='formlg' action='recuperar_password.php'>
    		...
    		echo"<form method='post' id='formlg' action='login_professors.php'>
    		...
    		echo"<form method='post' id='formlg' action='recuperar_password.php'>
    		<div class='form-group'><center><label>Recuperació contrasenya</label></center></br></br><label>Codi: </label><input class='form-control' type='text' name='codi'  required></br></br><label>Contrasenya nova: </label><input class='form-control' type='password' name='contrasenya'  required></div>
