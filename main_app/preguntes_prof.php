...
<body>
    ...
    <h1><strong><center>Preguntes</center></strong></h1>
  ...
  <tbody>
    <?php
     require 'conn.php';
                                    ...
                                    $cursos ="SELECT count(titol) as total from pregunta where IDprofessor=$idprof ";
      ...
      require 'conn.php';
        $sql = "SELECT p.titol, p.Data, p.tancada, p.id, al.NomUsuari, p.missatge as usuarialumne from pregunta p INNER JOIN professor a ON p.IDprofessor = a.ID INNER JOIN alumne al ON p.IDalumne = al.ID where p.IDprofessor=$idprof order by p.Data DESC LIMIT $pag, 10";
            ...
            while($row = $result->fetch_assoc()) {
                  echo"<tr>
                      ...
                      if($row["tancada"] == 0){
                        echo"<td>Oberta</td>";
                        echo"<td>
                      <form method='post' action='preguntadetallprof.php'>
                      <input type='hidden' name='idpregunta' value='".$row["id"]."'>
                      <input type='submit' class='btn btn-danger form-btn' value='Veure'>
                      </form>
                      </td>
                      </tr>";
                      }else{
  ...
  <tbody id="myTable">
    <?php
      require 'conn.php';
        $sql = "SELECT p.titol, p.Data, p.tancada, p.id, al.NomUsuari, p.missatge as usuarialumne from pregunta p INNER JOIN professor a ON p.IDprofessor = a.ID INNER JOIN alumne al ON p.IDalumne = al.ID where p.IDprofessor=$idprof order by p.Data DESC";
        $result = $mysqli->query($sql);
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                  echo"<tr>
                      <td>".utf8_encode($row["titol"])."</td>
                      <td>".$row["Data"]."</td>
                      <td>".utf8_encode($row["usuarialumne"])."</td>";
                      if($row["tancada"] == 0){
                        echo"<td>Oberta</td>";
                      }else{
                        echo"<td>Tancada</td>";
