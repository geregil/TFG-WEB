                ...
                <div class="col p-0 m-0">
                    <div class="row bg-white mx-auto">
                        <div class="col-12 p-0 m-0 bg-primary">
                            <?php
                                    require 'conn.php';
                                       $sql = "SELECT * FROM curs where ID = '".$_POST["idcurs"]."'";
                        ...
                        <div class="col-6 col-sm-6 col-md-6 col-lg-6 col-xl-6 p-0 m-0 p-3 text-secondary">
                            ...
                            <tbody>
                                <tr>
                                    <?php
                                     require 'conn.php';
                                       $sql = "SELECT COUNT(*) AS alumnes FROM alumne_curs a INNER JOIN curs c ON c.ID = a.IDcurs where a.IDcurs ='".$_POST["idcurs"]."'";
                                        $result = $mysqli->query($sql);
                                        if ($result->num_rows > 0) {
                                            while($row = $result->fetch_assoc()) {
                                                 $alumnes = $row["alumnes"];
                                ...
                                <tr>
                                    <?php
                                     require 'conn.php';
                                       $sql = "SELECT COUNT(*) AS tasques FROM curs c INNER JOIN tasca t ON c.ID = t.IDcurs where c.ID ='".$_POST["idcurs"]."'";
                                        $result = $mysqli->query($sql);
                                        if ($result->num_rows > 0) {
                                            while($row = $result->fetch_assoc()) {
                                                 $tasques = $row["tasques"];
                                ...
                                <tr>
                                    <?php
                                     require 'conn.php';
                                       $sql = "SELECT COUNT(*) AS exercicis, SUM(Punts) as punts FROM curs c INNER JOIN exercici e ON c.ID = e.IDcurs where c.ID ='".$_POST["idcurs"]."'";
                                        $result = $mysqli->query($sql);
                                        if ($result->num_rows > 0) {
                                            while($row = $result->fetch_assoc()) {
                                       ...
                                       $mysqli->close();
                                        ...
                                        require 'conn.php';
                                            $sql3 = "SELECT COUNT(al.ID) AS exercicistotals FROM alumne_exercici al INNER JOIN exercici e ON al.IDexercici = e.ID where al.IDalumne=$alumne and al.Punts_Obtinguts > 0 and e.IDcurs = '".$_POST["idcurs"]."'";
                                        ...
                                        require 'conn.php';
                                            $sql3 = "SELECT Sum(al.Punts_Obtinguts) AS punts FROM alumne_exercici al INNER JOIN exercici e ON al.IDexercici = e.ID where al.IDalumne=$alumne and e.IDcurs = '".$_POST["idcurs"]."'";
                                            $result = $mysqli->query($sql3);
                                            if ($result->num_rows > 0) {
                                                while($row = $result->fetch_assoc()) {
                                                    $puntsalumne = $row["punts"];
                                                }
                                            }
                                        $mysqli->close();
                                    ...
                                    ?>
                                    <td>Exercicis: <?php echo "$exercicisalumne / $exercicis" ?></a></td>
                                    <td class="p-0 m-0 p-1"></td>
                                </tr>
                                <tr>
                                    <td>Punts Totals: <?php echo "$puntsalumne / $punts" ?></td>
                                    <td class="p-0 m-0 p-1"></td>
                                </tr>
                                <tr>
                                    <?php
                                     require 'conn.php';
                                       $sql = "SELECT COUNT(a.ID) AS intents FROM alumne_exercici a INNER JOIN exercici e ON a.IDexercici = e.ID where a.IDalumne = $alumne and e.IDcurs ='".$_POST["idcurs"]."'";
                                        $result = $mysqli->query($sql);
                                        if ($result->num_rows > 0) {
                                            while($row = $result->fetch_assoc()) {
                                                 $intents = $row["intents"];
                ...
                <div class="col-6 col-sm-6 col-md-6 col-lg-6 col-xl-6 p-0 m-0 p-3 bg-warning mx-auto" style='color:white;border:1px solid black'>
                    ...
                    <div class="col-6 col-sm-6 col-md-6 col-lg-6 col-xl-6 p-0 m-0 p-3 bg-warning mx-auto" style='color:white;border:1px solid black'>
                            <h3><strong>Ha estrenat el curs</strong></h3>
                                 <?php
                                 require 'conn.php';
                                    $sql = "SELECT al.NomUsuari FROM alumne_exercici a INNER JOIN exercici e ON a.IDexercici = e.ID INNER JOIN alumne al ON a.IDalumne = al.ID where e.IDcurs =$idcurs and a.revisat = 1 GROUP BY a.IDalumne ORDER BY a.Data ASC LIMIT 1";
                                    $result = $mysqli->query($sql);
                                    if ($result->num_rows > 0) {;
                                        while($row = $result->fetch_assoc()) {
                                            echo $row["NomUsuari"];
        ...
        <div class="container"><img id="logo" src="../assets/img/logo.png"><a class="navbar-brand" href="perfil_alumne.php"><strong>El Racó d'estudi</strong></a><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button><a href="perfil_alumne.php"><img src="../imatges/icones/perfil.png" alt="Perfil" height="42" width="42"></a>
            <div
                ...
                <ul class="nav navbar-nav ml-auto">
                  <a href="estadistiquesalumne.php"><img src="../imatges/icones/torna.png" alt="Perfil" height="42" width="42" href="perfil_alumne.php"></a>
